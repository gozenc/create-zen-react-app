...
async function main() {
    ...
    try {
        ...
        installer.on("exit", () => {
            ...
            fs.rmSync(path.join(projectPath, 'docs'), { recursive: true, force: true });
            fs.rmSync(path.join(projectPath, 'bin'), { recursive: true, force: true });
            fs.rmSync(path.join(projectPath, '.npmignore'), { recursive: true, force: true });
            execSync(`git init`);
            console.log('Installation is done, ready to zen!');
            console.log('');
            console.log(`Just run "cd ${projectName} && ${hasYarn ? "npm run" : "yarn" } dev" to start development!`);
        });
    } catch (error) {
        console.log(error);
        process.exit(1);
